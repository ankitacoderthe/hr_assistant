...
const TotalFinesApproved = () => {
  ...
  function OverAllData() {
    ...
    let getString = url + "api/getFines?limit="+limit+"&offset="+offset+"&status='Approved'"
    if (date?.length>0){
    ...
    getString = url + "api/getFines?"+"&status='Approved'"
    if (date?.length>0){
        let from_date=moment(date[0])
...
export default TotalFinesApproved