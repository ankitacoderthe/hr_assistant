...
const AllOut = () => {
  ...
  useEffect(() => {
    ...
    let getString =url+"api/getTimings?in_time="+null+"&out_time="+!null+"&limit="+limit+"&offset="+offset
    if (date?.length > 0) {
    ...
    getString = url+"api/getTimings?in_time="+null+"&out_time="+!null
    if (date?.length > 0) {
      let from_date = moment(date[0])
      let to_date = moment(date[date.length - 1]).add(1, 'd')
      getString += "&from_date=" + from_date.format("YYYY-MM-DD") + "&to_date=" + to_date.format("YYYY-MM-DD")
  ...
  //  'floor_name'
  const tableKeys = [
    'employee_name', 'empID', 'out_time', 'location_name'
  ]
