...
const AllPresent = () => {
  ...
  const fetchData=()=>{
    let getString = url + "api/getAttendance?" + "&limit=" + limit + "&offset=" + offset+"&status='On Leave'"
    if (date?.length > 0) {
   ...
   getString = url + "api/getAttendance?&status='On Leave'"
   if (date?.length > 0) {
    let from_date = moment(date[0])
    let to_date = moment(date[date.length - 1]).add(1, 'd')
    getString += "&from_date=" + from_date.format("YYYY-MM-DD") + "&to_date=" + to_date.format("YYYY-MM-DD")
